...
Template.mainDashboard.events({
  "click #add": function (event, template) {
    ...
    if (apiAddress.find({ apiAddress: apiAddress1 }).count() > 0) {
      ...
      var selectedServer = sessionStorage.getItem('serverSelected');
      console.log(frequency);

      Meteor.call("updateApi", apiName, apiAddress1, getOrPost, usageOrStatus, authentication, frequency, selectedServer, function (error, result) {
        if (error) {
...
Template.mainDashboard.helpers({
  apis: function () {
    var selectedServer = sessionStorage.getItem('selectedServer');console.log(sessionStorage.getItem('serverSelected'));
    console.log(apiAddress.find({server: sessionStorage.getItem('serverSelected')}).fetch());

    var apiAddress1 = apiAddress.find({server: sessionStorage.getItem('serverSelected')}).fetch();
    var final = [];
