...
Meteor.methods({
  'updateApi': function (name, address, getOrPost, usageOrStatus, authentication, frequency,selectedServer) {
      ...
      if (getOrPost === "GET") {
        try {
          ...
          if (authentication) {
            ...
            try {
              apiAddress.insert({ createdBy: currentUser, apiName: name, apiAddress: address, authentication: authentication, getOrPost: getOrPost, usageOrStatus: usageOrStatus, response: res.data, updatedTime: postingTime, headers: res.headers, status: "pass", frequency: frequency, responseTime: responseTime, server: selectedServer });
  ...
  "addServerNames": function(serverName, userId){
    console.log('add names');
    try{
      console.dir('termainal');
      serverNameCollection.insert({createdBy: userId, serverName: serverName});
    }
    catch (err) {console.log('sdasd');
      serverNameCollection.insert({createdBy: userId, serverName: serverName});
    }
  }
});
